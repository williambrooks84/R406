...
import { CardStyleProps } from "../../interfaces/styleDefinitions";
import { CardDataProps } from "../../interfaces/dataDefinitions";

type Props = CardStyleProps & CardDataProps;

export default function Card({ badgeStyle, badgeData }: Props) {
  return (
    <div className="flex items-center justify-center border-2 border-amber-400 p-6">
      <Badge variant={badgeStyle?.variant} size={badgeStyle?.size} border={badgeStyle?.border}>
        {badgeData?.children}
      </Badge>
    </div>
  );
